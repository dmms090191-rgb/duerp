...
const PaymentSuccess3x: React.FC = () => {
  ...
  const processPayment = async () => {
    try {
      ...
      let employeeRange = searchParams.get('employee_range');

      if (!clientId || !employeeRange) {
        const pendingPayment = localStorage.getItem('stripy_payment_pending');
        if (pendingPayment) {
          try {
            ...
            if (age < 30 * 60 * 1000) {
              ...
              employeeRange = paymentData.employeeRange;
              localStorage.removeItem('stripy_payment_pending');
            }
          } catch (e) {
      ...
      console.log('📧 Paiement 3x confirmé - Génération des documents...');
...
export default PaymentSuccess3x;
