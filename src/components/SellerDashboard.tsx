...
const SellerDashboard: React.FC<SellerDashboardProps> = ({ sellerData, onLogout, onReturnToAdmin, onClientLogin }) => {
  ...
  const refreshClientsAndModal = async () => {
    console.log('🔄 refreshClientsAndModal - Starting...');
    await fetchClients();

    if (selectedClientDetails) {
      try {
        console.log('🔄 Reloading client details for ID:', selectedClientDetails.id);
        const { data, error } = await supabase
          .from('clients')
          .select('*')
        ...
        if (!error && data) {
          console.log('✅ Client reloaded - diagnostic_final_actif:', data.diagnostic_final_actif);
        ...
        console.error('❌ Exception lors du rechargement du client:', error);
      }
    }
  };
  ...
  const handleSaveClient = async () => {
    ...
    try {
      const updates = {
        ...
        diagnostic_final_actif: editedClient.diagnostic_final_actif ?? false,
        type_diagnostic: editedClient.type_diagnostic || null
      };
