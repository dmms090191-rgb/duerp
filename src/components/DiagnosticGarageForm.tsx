...
const DiagnosticGarageForm: React.FC<DiagnosticGarageFormProps> = ({ clientId, typeDiagnostic }) => {
  ...
  const [expandedSubQuestions, setExpandedSubQuestions] = useState<Record<string, boolean>>({});
  ...
  const setRiskStatus = (questionId: string, categoryId: string, status: 'maitrise' | 'non_maitrise' | 'non_applicable') => {
    ...
    const catIndex = duerpCategories.findIndex(c => c.id === categoryId);
    const cat = duerpCategories[catIndex];
    if (cat && cat.questions[0]?.isYesNoOnly && cat.questions[0].id === questionId) {
      const isOui = status === 'oui';
      const expandUpdates: Record<number, boolean> = { [catIndex]: isOui };
      duerpCategories.forEach((c, i) => {
        if (c.parentGateQuestionId === questionId) {
          expandUpdates[i] = isOui;
        }
      });
      setExpandedSidebarCategories(prev => ({ ...prev, ...expandUpdates }));
    }
  };
            ...
            <div className="flex-1 flex overflow-hidden relative">
                          ...
                          {(isSidebarExpanded && hasExpandableQuestions) && (
                            <div className="ml-2 mt-2 space-y-2">
                              {(catHasGate ? cat.questions.slice(1) : cat.questions).map((question, filteredIndex) => {
                                ...
                                const parentDisplayNumber = question.displayNumber || '';
                                const isSubQuestion = parentDisplayNumber.split('.').length >= 3;
                                if (isSubQuestion) {
                                  const parentPrefix = parentDisplayNumber.split('.').slice(0, 2).join('.');
                                  const parentKey = `${catIdx}_${parentPrefix}`;
                                  if (!expandedSubQuestions[parentKey]) return null;
                                }

                                const isSubParentExpanded = expandedSubQuestions[`${catIdx}_${parentDisplayNumber}`];
                                    ...
                                    <button
                                      onClick={() => {
                                        if (question.hasSubQuestions) {
                                          const key = `${catIdx}_${parentDisplayNumber}`;
                                          setExpandedSubQuestions(prev => ({ ...prev, [key]: !prev[key] }));
                                          setSelectedDuerpCategory(catIdx);
                                      ...
                                      className={`group relative w-full text-left px-4 py-3 rounded-xl text-sm transition-all duration-300 overflow-hidden ${
                                        ...
                                        isActive && viewMode === 'question' && !question.hasSubQuestions
                                          ...
                                          : question.hasSubQuestions
                                            ? 'bg-sky-100 text-gray-700 hover:bg-sky-200 border border-sky-300 hover:border-sky-400 hover:shadow-lg'
                                      ...
                                      <div className="relative flex items-start gap-3">
                                        ...
                                        <div className="flex-1">
                                          <p className={`font-semibold leading-snug mb-1.5 ${
                                            isActive && viewMode === 'question' && !question.hasSubQuestions ? 'text-white' : 'text-gray-800 group-hover:text-gray-900'
                                          }`}>
                                            {question.text}
                                          </p>
                                        ...
                                        {question.hasSubQuestions && (
                                          <div className={`flex-shrink-0 w-6 h-6 rounded-md flex items-center justify-center mt-1 transition-transform duration-300 ${
                                            isSubParentExpanded ? 'rotate-180' : ''
                                          } ${
              ...
              <div className="flex-1 overflow-y-auto">
                ...
                {!duerpCategories || duerpCategories.length === 0 ? (
                  ...
                  const parentSubDescription = (() => {
                    if (!currentDuerpQuestion) return null;
                    if (currentDuerpQuestion.hasSubQuestions && currentDuerpQuestion.subDescription) {
                      return { title: currentDuerpQuestion.text, description: currentDuerpQuestion.subDescription };
                    }
                    const dn = currentDuerpQuestion.displayNumber || '';
                    const parts = dn.split('.');
                    if (parts.length >= 3) {
                      const parentPrefix = parts.slice(0, 2).join('.');
                      const parent = currentCategory.questions.find(
                        (q: any) => q.displayNumber === parentPrefix && q.hasSubQuestions && q.subDescription
                      );
                      return parent ? { title: parent.text, description: parent.subDescription } : null;
                    }
                    return null;
                  })();
                  ...
                  if (currentDuerpQuestion.hasSubQuestions && parentSubDescription) {
                    return (
                      <div className="max-w-5xl mx-auto min-h-[300px] sm:min-h-[600px]">
                        <div className="relative bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-xl p-4 mb-4 overflow-hidden border border-cyan-500/30 shadow-lg shadow-cyan-500/20">
                          ...
                          <div className="relative z-10 flex items-start gap-3">
                            <div className="relative w-8 h-8 rounded-lg bg-gradient-to-br from-cyan-500 via-blue-500 to-blue-600 flex items-center justify-center flex-shrink-0 shadow-lg shadow-cyan-500/50 border border-cyan-400/50">
                              <div className="absolute inset-0 rounded-lg bg-gradient-to-br from-white/30 to-transparent"></div>
                              <Shield className="w-4 h-4 text-white relative z-10 drop-shadow-lg" />
                            </div>
                            <div className="flex-1">
                              <h2 className="text-lg sm:text-xl font-black bg-gradient-to-r from-cyan-400 via-blue-400 to-cyan-400 bg-clip-text text-transparent leading-tight drop-shadow-lg mb-1">
                                {currentDuerpQuestion.displayNumber} - {parentSubDescription.title}
                              </h2>
                            </div>
                          </div>
                        </div>

                        <div className="relative bg-gradient-to-br from-sky-50 to-blue-50 rounded-xl p-5 sm:p-6 border border-sky-200/80 shadow-sm">
                          <div className="flex items-start gap-4">
                            <div className="w-1.5 self-stretch rounded-full bg-gradient-to-b from-sky-400 to-blue-500 flex-shrink-0"></div>
                            <div>
                              <p className="text-sm sm:text-base text-slate-700 leading-relaxed">{parentSubDescription.description}</p>
                              <p className="mt-4 text-xs text-slate-500 italic">Cliquez sur les sous-questions dans le menu de gauche pour commencer l'evaluation.</p>
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  return (
                    <div className="max-w-5xl mx-auto min-h-[300px] sm:min-h-[600px]">
                      ...
                      {parentSubDescription && (
                        <div className="relative bg-gradient-to-br from-sky-50 to-blue-50 rounded-xl p-4 mb-3 border border-sky-200/80 shadow-sm">
                          <div className="flex items-start gap-3">
                            <div className="w-1 self-stretch rounded-full bg-gradient-to-b from-sky-400 to-blue-500 flex-shrink-0"></div>
                            <div>
                              <h3 className="text-sm font-bold text-slate-800 mb-1">{parentSubDescription.title}</h3>
                              <p className="text-sm text-slate-600 leading-relaxed">{parentSubDescription.description}</p>
                            </div>
                          </div>
                        </div>
                      )}
