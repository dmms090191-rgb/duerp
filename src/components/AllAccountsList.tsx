...
import { Users, Shield, ShoppingBag, User, Mail, Phone, Calendar, Search, Filter, Edit, MessageSquare, Briefcase, Building2, Hash, FileText, Unlock, LogIn } from 'lucide-react';
import SectorUnlockManager from './SectorUnlockManager';
...
interface AllAccountsListProps {
  ...
  onClientLogin?: (clientId: number, clientEmail: string) => void;
}

const AllAccountsList: React.FC<AllAccountsListProps> = ({ onOpenChat, onClientLogin }) => {
  const [accounts, setAccounts] = useState<Account[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [filterType, setFilterType] = useState<'all' | 'admin' | 'seller' | 'client'>('client');
  const [selectedClientForUnlock, setSelectedClientForUnlock] = useState<{ id: number; name: string } | null>(null);
  ...
  return (
    <div className="space-y-6">
                      ...
                      <>
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2 flex-wrap">
                            <button
                              onClick={() => setSelectedClientForUnlock({ id: parseInt(account.id), name: account.full_name })}
                              ...
                              title="Gérer les secteurs débloqués"
                            >
                              <Unlock className="w-3 h-3" />
                              Secteurs
                            </button>
                            <button
                              onClick={() => onClientLogin?.(parseInt(account.id), account.email)}
                              ...
                              title="Se connecter au panel client"
                            >
      ...
      {selectedClientForUnlock && (
        <SectorUnlockManager
          clientId={selectedClientForUnlock.id}
          clientName={selectedClientForUnlock.name}
          onClose={() => setSelectedClientForUnlock(null)}
        />
      )}
    </div>
  );
